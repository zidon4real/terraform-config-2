...
module "VPC" {
  ...
  # Adjust the CIDR blocks for public and private subnets
  private_subnets = [for i in range(1, 8, 2) : cidrsubnet(var.vpc_cidr, 8, i+10)]
  public_subnets  = [for i in range(2, 5, 2) : cidrsubnet(var.vpc_cidr, 8, i+10)]
}
